...
const AddRemoveCard = ({ name, username, showCard, toggleCard }) => {
  ...
  `;
    ...
    const fetchUserData = async variables => dataFetch({ query: userQuery, variables });
    ...
    function handleGitLabChange(event) {
        ...
        if(changedPlatforms.indexOf(event.target.name) !== -1){
            ...
            setChangedPlatforms(changedPlatforms => [...changedPlatforms, event.target.name]);
        }
    }
    ...
    return showCard ? (
        <React.Fragment>
            ...
            <div className="confirm-modal-wrapper" aria-modal aria-hidden role="dialog">
                <div className="addremove-confirm-modal">
                    <div className="confirm-modal-header"><h5>{name}</h5></div>
                    <label>Please select/deselect the below platforms to add/remove users</label>
                    {
                        username ?
                            loadPlatformData ?
                                <div>
